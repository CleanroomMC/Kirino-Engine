 ...
 * </ul>
 */
public class Texture1DArrayAccessor extends TextureAccessorExt implements TextureAccessorHighlevel {

    public final GLTexture texture;

    public Texture1DArrayAccessor(boolean dsa, GLTexture texture) {
        super(dsa);
        Preconditions.checkState(texture.type == TextureType.TEX_1D_ARRAY,
                "Texture type must be TEX_1D_ARRAY.");
    ...
    public TextureType type() {
        return TextureType.TEX_1D_ARRAY;
    }
    ...
    private static class HighlevelOperatorImpl implements TextureAccessorHighlevel.HighlevelOperator {
        ...
        private final Texture1DArrayAccessor accessor;

        private HighlevelOperatorImpl(Texture1DArrayAccessor accessor) {
        ...
        public void resizeAndAllocEmpty(int width, int height) {
            ...
            MethodHolder.setLayers(accessor.texture, height);
        ...
        public void alloc(boolean mutable, @NonNull ByteBuffer byteBuffer, @NonNull TextureFormat format) {
            ...
            if (mutable) {
                accessor.texImage2D(0, format.internalFormat, accessor.texture.extentX(), accessor.texture.layers(), 0, format.format, format.type, byteBuffer);
            } else {
                accessor.texStorage2D(1, format.internalFormat, accessor.texture.extentX(), accessor.texture.layers());
                accessor.texSubImage2D(0, 0, 0, accessor.texture.extentX(), accessor.texture.layers(), format.format, format.type, byteBuffer);
            }
        }
        ...
        public void allocEmpty(boolean mutable, @NonNull TextureFormat format) {
            ...
            if (mutable) {
                accessor.texImage2D(0, format.internalFormat, accessor.texture.extentX(), accessor.texture.layers(), 0, format.format, format.type, null);
            } else {
                accessor.texStorage2D(1, format.internalFormat, accessor.texture.extentX(), accessor.texture.layers());
            }
        }
        ...
        public void alloc(@NonNull StorageOptions options, @NonNull ByteBuffer data, @NonNull TextureFormat format, boolean generateMipmaps) {
            ...
            int height = accessor.texture.layers();
            ...
            if (options.mutable()) {
                if (generateMipmaps) {
                    ...
                    for (int level = 0; level < levels; level++) {
                        accessor.texImage2D(level, format.internalFormat, mipExtent(width, level), height, 0, format.format, format.type, level == 0 ? data : null);
                    }
                }
            } else {
        ...
        public void allocEmpty(@NonNull StorageOptions options, @NonNull TextureFormat format) {
            ...
            if (options.mutable()) {
                for (int level = 0; level < levels; level++) {
                    accessor.texImage2D(level, format.internalFormat, mipExtent(width, level), height, 0, format.format, format.type, null);
                }
            } else {
                accessor.texStorage2D(levels, format.internalFormat, width, height);
            }
        ...
        public void uploadLevel(int level, @NonNull ByteBuffer data, @NonNull TextureFormat format) {
            ...
            accessor.texSubImage2D(level, 0, 0, mipExtent(accessor.texture.extentX(), level), accessor.texture.layers(), format.format, format.type, data);
        }
