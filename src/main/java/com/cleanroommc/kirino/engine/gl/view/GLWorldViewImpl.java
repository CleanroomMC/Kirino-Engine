package com.cleanroommc.kirino.engine.gl.view;
...
import com.cleanroommc.kirino.engine.world.context.GLWorldView;
...
public class GLWorldViewImpl implements GLWorldView {
    ...
    private final Map<FramePhase, Consumer<WorldContext<GL>>> callbacks = new HashMap<>();

    public GLWorldViewImpl(
            CleanECSRuntime ecs,
    ...
    public void run(@NonNull FramePhase phase) {
        Consumer<WorldContext<GL>> consumer = callbacks.get(phase);
        if (consumer != null) {
        ...
        switch (phase) {
            ...
            case RENDER_OPAQUE -> {
                ...
                rs().terrainGpuPass.render(sceneViewState.camera);
//                rs().chunkCpuPass.render(sceneViewState.camera);
            ...
            case RENDER_TRANSPARENT -> {
                ...
                rs().gizmosPass.render(sceneViewState.camera);
    ...
    @Override
    public void on(@NonNull FramePhase phase, @NonNull Consumer<WorldContext<GL>> consumer) {
